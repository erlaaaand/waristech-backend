...
@Injectable()
export class GetAllocatedAssetsAhliWarisUseCase {
  constructor(
    @Inject(ASSET_REPOSITORY_TOKEN)
    private readonly repository: IAssetRepository,
  ) {}

  async execute(ahliWarisId: string): Promise<AssetResponseDto[]> {
    const allocatedAssets =
      await this.repository.findByAhliWarisId(ahliWarisId);

    return allocatedAssets.map((asset) => this.toResponseDto(asset));
  }
